...
app.get("/", (req, res) => {
  fs.readFile(indexPath, "utf8", (err, htmlData) => {
    ...
    htmlData = htmlData
      .replace("__META_OG_TITLE__", `Welcome to Fibit Pro`)
      .replace("__META_OG_DESCRIPTION__", `Welcome to Fibit Pro`)
      .replace("__META_DESCRIPTION__", `Welcome to Fibit Pro`)
      .replace(
        "__META_OG_IMAGE__",
      ...
      .replace("__META_TWITTER_TITLE__", `Welcome to Fibit Pro`)
      .replace("__META_TWITTER_DESCRIPTION__", `Welcome to Fibit Pro`)
      .replace(
...
app.get("/nftDetails/:id", (req, res, next) => {
  fs.readFile(indexPath, "utf8", (err, htmlData) => {
    ...
    let url = `http://api.3.67.57.47.nip.io/api/v1/nft/single/${req.params.id}`;
    axios
      ...
      .then((ress) => {
        ...
        htmlData = htmlData
          .replace(
            "<title>Welcome to Fibit Pro</title>",
...
app.get("/creator/:id", (req, res, next) => {
  fs.readFile(indexPath, "utf8", (err, htmlData) => {
    ...
    /* axios api call */
    let header = { "content-type": "application/json" };
    let url = `http://api.3.67.57.47.nip.io/api/v1/user/getSingleUser/${req.params.id}`;
    axios
      .get(url, { headers: header })
      .then((ress) => {
        // inject meta tags
